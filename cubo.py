...
class Cubo:
    ...
    idxNorte = [
        [2, 1, 0],  # casillas fronterizas al Norte para cara 0
        [0, 7, 6],  # idem cara 1
        [6, 5, 4],  # idem cara 2
        [4, 3, 2],  # idem cara 3
        [2, 1, 0],  # idem cara 4
    ...
    #  a mover en el giro normal (sentido del reloj visto de frente)
    idxEste = [
        [2, 1, 0],  # casillas fronterizas al Este para cara 0
        [0, 7, 6],  # idem cara 1
        [0, 7, 6],  # idem cara 2
        [0, 7, 6],  # idem cara 3
        [0, 7, 6],  # idem cara 4
    ...
    #  a mover en el giro normal (sentido del reloj visto de frente)
    idxSur = [
        [2, 1, 0],  # casillas fronterizas al Sur para cara 0
        [0, 7, 6],  # idem cara 1
        [2, 1, 0],  # idem cara 2
        [4, 3, 2],  # idem cara 3
        [6, 5, 4],  # idem cara 4
    ...
    # a mover en el giro normal (sentido del reloj visto de frente)
    idxOeste = [
        [2, 1, 0],  # casillas fronterizas al Oeste para cara 0
        [4, 3, 2],  # idem cara 1
        [4, 3, 2],  # idem cara 2
        [4, 3, 2],  # idem cara 3
        [4, 3, 2],  # idem cara 4
    ...
    def girarHorario(self, idxCara: int) -> None:
        ...
        for i in range(0, 3):
            # Norte -> Este
            aux1 = self.caras[self.vecinoEste[idxCara]].casillas[
                self.idxEste[idxCara][i]
            ]
            self.caras[self.vecinoEste[idxCara]].casillas[self.idxEste[idxCara][i]] = (
                self.caras[self.vecinoNorte[idxCara]].casillas[
                    self.idxNorte[idxCara][i]
                ]
            )
            ...
            # Sur -> Oeste
            aux3 = self.caras[self.vecinoOeste[idxCara]].casillas[
                self.idxOeste[idxCara][i]
            ]
            self.caras[self.vecinoOeste[idxCara]].casillas[
                self.idxOeste[idxCara][i]
            ] = aux2
    ...
    def girarAntiHorario(self, idxCara: int) -> None:
        ...
        for i in range(0, 3):
            # Norte -> Oeste
            aux1 = self.caras[self.vecinoOeste[idxCara]].casillas[
                self.idxOeste[idxCara][i]
            ]
            self.caras[self.vecinoOeste[idxCara]].casillas[
                self.idxOeste[idxCara][i]
            ...
            # Sur -> Este
            aux3 = self.caras[self.vecinoEste[idxCara]].casillas[
                self.idxEste[idxCara][i]
            ]
            self.caras[self.vecinoEste[idxCara]].casillas[
                self.idxEste[idxCara][i]
            ] = aux2
    ...
    def visualizar(self) -> str:
        ...
        # Caras 1, 2, 3, 4
        resultado += (
            self.stringFila1(self.caras[1])
            + " "
            + self.stringFila1(self.caras[2])
            + " "
            + self.stringFila1(self.caras[3])
            + " "
            + self.stringFila1(self.caras[4])
            + "\n"
            + self.stringFila2(self.caras[1])
            + " "
            + self.stringFila2(self.caras[2])
            + " "
            + self.stringFila2(self.caras[3])
            + " "
            + self.stringFila2(self.caras[4])
            + "\n"
            + self.stringFila3(self.caras[1])
            + " "
            + self.stringFila3(self.caras[2])
            + " "
            + self.stringFila3(self.caras[3])
            + " "
            + self.stringFila3(self.caras[4])
            + "\n\n"
        )
